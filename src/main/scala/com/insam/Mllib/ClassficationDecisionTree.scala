...
object ClassficationDecisionTree {
  val spark = SparkSession.builder().master("local").getOrCreate()

  // Load the data stored in LIBSVM format as a DataFrame.
  val data = spark.read.format("libsvm").load("/home/jjh/문서/dataset/spark_mllib/sample_libsvm_data.txt")
  ...
  predictions.select("predictedLabel", "label", "features").
    filter($"label" =!= $"predictedLabel").show()
